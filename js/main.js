...
function user_register(){
  ...
  if(is_error==''){
    jQuery.ajax({
      ...
      success: function(result){
        if(result=='valid'){
          $('#success p').html('Thank you registation');
        }
        if(result=='wrong'){
          $('#email_error').html('Email allready exist');
        }
      }
    });
  }
...
function user_login(){
  ...
  if(is_error==''){
    jQuery.ajax({
      ...
      success: function(result){
        if(result=='valid'){
          $('.login_msg').html("<p style='color:green;margin-top:10px'>Login successfull</p>");
          window.location.href=window.location.href;
        }
        if(result=='wrong'){
          $('.login_msg').html("<p style='color:red;margin-top:10px'>Please enter valid information</p>");
        }
      }
    });
  }
